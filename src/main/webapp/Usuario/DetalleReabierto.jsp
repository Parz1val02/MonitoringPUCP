    ...
    <body>
        ...
        <div class="container" style="margin-top: 50px">
            <div class="mb-3">
                ...
                <input type="text" class="form-control" name="nombreIncidencia"
                       value="<%=incidencia.getNombreIncidencia()%>" aria-label="Disabled input example" disabled>
            </div>

            <div class="mb-3">
                <label class="form-label">Tipo Incidencia:</label>
                <input type="text" class="form-control" name="tipoIncidencia"
                       value="<%=incidencia.getTipoIncidencia().getTipo()%>" aria-label="Disabled input example"
                       disabled>
            </div>

            <div class="mb-3">
                <label class="form-label">Zona PUCP:</label>
                <input type="text" class="form-control" name="zonaPUCP"
                       value="<%=incidencia.getZonaPUCP().getNombreZona()%>" aria-label="Disabled input example"
                       disabled>
            </div>

            <div class="mb-3">
                <label class="form-label">Nivel de urgencia:</label>
                <input type="text" class="form-control" name="nivelDeUrgencia"
                       value="<%=incidencia.getNivelUrgencia().getNivel()%>" aria-label="Disabled input example"
                       disabled>
            </div>

            <div class="mb-3">
                <label class="form-label">Descripción de la incidencia:</label>
                <input type="text" class="form-control" name="descripcionIncidencia"
                       value="<%=incidencia.getDescripcion()%>" aria-label="Disabled input example" disabled>
            </div>
        ...
        <div class="mb-3">
            <div class="container">
                <form method="post"
                      action="<%=request.getContextPath()%>/UsuarioServlet?accion=reabrir&id=<%=incidencia.getIdIncidencia()%>">
                    <div class="form-floating" style="margin-left: 10px">
                        <p>
                            Justificación de la incidencia: <br>
                            <label for="floatingTextarea2"></label>
                            <textarea class="form-control" placeholder="<%=incidencia.getDescripcion()%>"
                                      style="height: 100px" aria-label="Disabled input example" disabled
                                      readonly></textarea>
                        </p>
                    </div>
                    <div class="form-floating" style="margin-left: 10px">
                        ...
                        <label for="floatingTextarea2"></label>
                        <textarea class="form-control <%=validoComentario.length()>0?"is-invalid":""%>"
                                  placeholder="Deja un comentario aquí" id="floatingTextarea2" name="reopen"
                    ...
                    <div class="form-group">
                        <button class="btn btn-danger"><%=session.getAttribute("info")%>
                        </button>
                        <a href="<%=request.getContextPath()%>/UsuarioServlet?accion=listar" class="btn btn-secondary">Regresar
                            a la ventana principal</a>
                    </div>
                    <%} else {%>
                    <div class="form-group">
